...
for(var i = 0 ; i < 110 ; i++){
    ...
    r =  Math.random()*Math.random()*120+20
...
console.log(111)
for (var i = 0; i < 100; i++) {
    posPoint = new Point(Math.random()*600,i*10);
    // maskCircle = new Path.Circle(posPoint,Math.random()*250);
    maskCircle = new Path.Rectangle(posPoint,Math.random()*400+140,10);
    fuga = fuga.unite(maskCircle)
    fuga.remove()
    maskCircle.remove()
}

var maskCircles = fuga
fuga.remove()
